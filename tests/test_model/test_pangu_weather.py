...
def test_pangu_weather_load_pretrained(batch_size, best_device, weather_statistics, constant_maps, const_h):
    ...
    pangu_weather_pangu_pytorch = pangu_pytorch_model.PanguWeather(
        ...
        constant_maps.to(best_device), const_h.to(best_device), dim, device='cpu').to(best_device)
...
def test_pangu_weather_vs_onnx_on_example_input(batch_size, best_device, weather_statistics, constant_maps, const_h,
    ...
    max_upper_error_by_variable = (upper_air_output - upper_air_output_onnx).abs().view(batch_size, 5, 13, -1).amax((0, -1))

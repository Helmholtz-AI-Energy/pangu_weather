...
def extract_auxiliary_data_from_onnx(onnx_path, overwrite=False):
    logger.info("Extracting auxiliary data from ONNX weights.")
    ...
    def save_aux_data(onnx_name, output_path):
        ...
        if output_path.exists():
            ...
            if saved_tensor.shape != aux_tensor.shape:
                ...
                message += ", but values are not allclose to new version."
            else:
                message += " and is identical to new version."
...
def convert_onnx_to_torch_checkpoint(onnx_path, torch_output_path, overwrite=False, device="cpu"):
    logger.info("Converting ONNX weights to torch checkpoint.")
    dim = 192
    ...
    if torch_output_path.exists():
        ...
        if (previous_checkpoint.keys() != checkpoint.keys()) or (
            ...
            message += ", but has mismatching keys."
        elif not all(
            torch.equal(checkpoint["model"][key], previous_checkpoint["model"][key])
...
def download_onnx_weights_and_example_input(overwrite=False):
    logger.info("Downloading ONNX weights and example input from official pangu-weather repository.")
    ...
    gdrive_urls = {
        pretrained_model_path_onnx: "https://drive.google.com/file/d/1lweQlxcn9fG0zKNW8ne1Khr9ehRTI6HP/view?usp=share_link",
        example_input_path
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--download", action="store_true", help="Download missing data (onnx weights and example inputs)"
    )
    parser.add_argument("--extract_aux_data", action="store_true", help="Extract auxiliary data from ONNX weights.")
    parser.add_argument("--to_torch", action="store_true", help="Convert ONNX weights to torch checkpoint.")
    ...
    if cli_args.to_torch:
        convert_onnx_to_torch_checkpoint(
            pretrained_model_path_onnx, pretrained_model_path_torch, overwrite=cli_args.overwrite
        )
